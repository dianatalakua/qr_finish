...
if (isset($_GET['qr_content']) && isset($_GET['matkul_id']) && isset($_GET['id_mahasiswa'])) {
    ...
    $queryAbsen = "SELECT * FROM absensi_mahasiswa WHERE id_mahasiswa = $id_mahasiswa AND id_matkul = $matkul_id";
    $resultAbsen= $conn->query($queryAbsen);

    if ($resultAbsen->num_rows > 0) {
        echo "<div class='container mt-5'>
            <h1>Kamu sudah scan QRcode, tunggu sampai Absen Berakhir</h1>
            </div>";
    } else {
        ...
        if ($resultMatkul && $resultMatkul->num_rows > 0) {
            ...
            if ($qr_content === "Matkul: $kodeMatkul\nNama: $namaMatkul\nKelas: $kelas\nHari: $hari\nJam: $jamMulai - $jamSelesai\nDosen: $id_dosen\nTime = $currentDate") {
                ...
                if ($resultAbsensi) {
                    ...
                    echo "<script>window.location.href='student_signature.php?matkul_id=$matkul_id&id_mahasiswa=$id_mahasiswa&kode_matkul=$kode_matkul';</script>";
                    exit();
                } else {
