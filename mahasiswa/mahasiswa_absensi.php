...
if (isset($_GET['matkul_id'])) {
    ...
    if ($resultMatkul && $resultMatkul->num_rows > 0) {
        ...
        echo "<video id='scanner' width='100%' style='transform: scaleX(-1);'></video>"; // Untuk mengakses kamera belakang, tambahkan transform: scaleX(-1);
        echo "</div>";
        echo "</div>";
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                if ('mediaDevices' in navigator && 'getUserMedia' in navigator.mediaDevices) {
                    ...
                    navigator.mediaDevices.getUserMedia({ video: { facingMode: { exact: 'environment' } } }) // Mengakses kamera belakang
                        .then(function (stream) {
                            ...
                            scanner.addListener('scan', function (content) {
                                ...
                                window.location.href = 'process_attendance.php?qr_content=' + encodeURIComponent(content) + '&matkul_id=<?php echo $matkul_id; ?>&id_mahasiswa=<?php echo $idMahasiswa; ?>&kode_matkul=<?php echo $kodeMatkul; ?>';
                            });
